...
class UserManagement(Frame):
    def __init__(self, view, window):
        ...
        self.prettytable_print = PrettyTable()
        ...
        self.print_data_tabel_pdf_button = None
    ...
    def load_data(self, current_user):
        ...
        if self.current_user.role_id == 1 or self.current_user.role_id == 2:
            if not self.admin_button or self.default_user_button or self.print_data_tabel_pdf_button:
                self.print_data_tabel_pdf_button = Button(self, text="Print Table Data", bootstyle=OUTLINE + INFO,
                                                          command=self.print_data_tabel_pdf_button_clicked)
                self.print_data_tabel_pdf_button.grid(row=0, column=0, columnspan=2, padx=(10, 10), pady=(10, 10))
            ...
            if self.admin_button and self.default_user_button and self.print_data_tabel_pdf_button:
                self.print_data_tabel_pdf_button.destroy()
    ...
    def print_data_tabel_pdf_button_clicked(self):
        result = self.user_business.pdf_data_users(self.current_user, self.sort)
        users = result[0]
        error_message = result[1]
        ...
        if error_message:
            ...
            with open("data.txt", mode="w") as file:
                row_number = 1
                for item in users:
                    self.prettytable_print.field_names = ["NO", "First Name", "Last Name", "Username", "Email",
                                                          "Gender", "Age", "Country", "City", "Status", "Role"]
            ...
            file_1 = open("user_data.txt", mode="r")
            for x in file_1:
                pdf.cell(190, 7, txt=x, ln=1, align="C", border=0, fill=False)
            pdf.output("user_data.pdf")
            file_1.close()
            Messagebox.show_info(title="Info", message="Your Data saved", alert=True)
