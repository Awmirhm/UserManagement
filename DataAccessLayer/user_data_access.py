...
class UserDataAccess:
    ...
    def data_log_to_pdf(self):
        with sqlite3.connect(self.data_base_logs, isolation_level=None,
                             detect_types=sqlite3.PARSE_COLNAMES) as connection:
            change_data = pandas.read_sql_query("""SELECT * FROM Logs""", connection)
            change_data.to_csv("database_log.csv", index=False)
        chart_data = pandas.read_csv("database_log.csv")
        chart_data.execution_time.plot(figsize=(8, 7), fontsize=16, c="r", linestyle="--")
        plt.style.use("seaborn-v0_8-dark-palette")
        plt.legend(loc=5)
        plt.title("Logs Execution Time", fontsize=15)
    ...
    def data_users_to_pdf(self, current_user_id):
        users = []
        with sqlite3.connect(self.data_base_name) as connection:
            cursor = connection.cursor()
            data = cursor.execute("""
