...
class UserBusiness:
    ...
    def get_users(self, current_user, sort):
        ...
        if current_user.active == 0:
            ...
            if sort == "Role":
                sort_by_role = sorted(users, key=lambda role: role.role_id)
                return [sort_by_role, None]
            return [users, None]
    ...
    def pdf_data_logs(self, current_user):
        if current_user.role_id != 1:
            return [None, "Invalid Access"]
        if current_user.active == 0:
            return [None, "Your Account Deactivate"]
        else:
            self.user_data_access.data_log_to_pdf()
            return ["Your Data saved", None]

    #
    def pdf_data_users(self, current_user, sort):
        if current_user.role_id != 1 and current_user.role_id != 2:
            return [None, "Invalid Access"]
        if current_user.active == 0:
            return [None, "Your Account Deactivate"]
        else:
            users = self.user_data_access.data_users_to_pdf(current_user.id)
            if sort == "Name":
                sort_by_name = sorted(users, key=lambda name: name.firstname)
                return [sort_by_name, None]
            if sort == "Gender":
